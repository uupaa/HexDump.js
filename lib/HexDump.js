...
//}@dev
    ...
    if (ENABLE_CONSOLE_STYLE && dumpRule) {
        ...
        for (var tag in dumpRule) {
            ...
            if ("begin" in dumpRule[tag] || "end" in dumpRule[tag]) {
            ...
            if (dumpRule[tag]["bold"]) {
                console.log("%c [" + (styleIndex++) + "] " + tag, dumpRule[tag]["style"] + ";font-weight:bold");
            } else {
...
//              consoleStyles.push("color:black");
    ...
    if (unitSize === 1 && String.prototype.repeat) {
        ...
        var remainRule = "";
        if (lineCSS.length) {
            remainRule = " ".repeat(16 - lineASCII.length + 1) + "[" + lineCSS.sort().join("][") + "]";
        }
        console.log.apply(console, [result.join("") + " ".repeat(padLength) + lineASCII.join("") + remainRule].concat(consoleStyles));
        return;
    }
    ...
    function _seriesOfElements(result, source, begin, end, values, cssText, bold) {
    ...
    function _range(result, source, begin, end, cssText, bold) {
